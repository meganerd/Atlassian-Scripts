...
callUrl "GET" "${jira_fqdn}/rest/api/2/serverInfo"
if [[ $? != 0 ]]; then
...
if [[ "$background" == "true" ]]; then
        ...
        callUrl "POST" "${jira_fqdn}/rest/api/2/reindex"
else
                echo "$(pdate) Starting Foreground re-index...."
        callUrl "POST" "${jira_fqdn}/rest/api/2/reindex?type=FOREGROUND"
fi
...
do
        sleep 5
        callUrl "GET" "${jira_fqdn}${progress_url}"
        if [[ $? != 0 ]];then
